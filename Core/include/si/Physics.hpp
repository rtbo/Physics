...
#if defined(HAVE_ENERGY_HPP) && defined(HAVE_MASS_HPP) && defined(HAVE_VELOCITY_HPP)
    constexpr Energy potentialEnergy(const Mass& m, const Length &h, const Acceleration& g=earth_gravity_t{ 1.0 })
    {
        return m * g * h;
    }
#endif
...
#if defined(HAVE_ACCELERATION_HPP) && defined(HAVE_DENSITY_HPP) && defined(HAVE_LENGTH_HPP) && defined(HAVE_PRESSURE_HPP)
    constexpr Pressure hydrostaticPressure(const Density& rho, const Length& h, const Acceleration& g=earth_gravity_t{ 1.0 })
    {
        return rho * g * h;
    }
#endif
