...
public:
    ...
    constexpr explicit unit() : _val{ 0.0 } {}

    constexpr explicit unit(const double val) : _val{ val } {}

    constexpr unit(const Value<D> &val) : _val{ C::reverse_conv(val._repr) } {}

    constexpr unit(const unit &val) = default;
    constexpr unit &operator=(const unit &val) = default;

    constexpr explicit operator double() const { return _val; }
    ...
    constexpr double repr() const { return C::conv(_val); }
};
...
template<typename U1, typename U2>
constexpr std::enable_if_t<are_units<U1, U2>(), bool> are_compatible_units()
{
...
inline constexpr auto operator*(const unit<LD, LC> &lhs,
                                const unit<RD, RC> &rhs)
{
...
template<typename D, typename C>
inline constexpr unit<D, C> clamp(const unit<D, C> &u, const unit<D, C> &umin,
                                  const unit<D, C> &umax)
{
...
inline constexpr Value<D> clamp(const Value<D> &u, const unit<D, C1> &umin,
                                const unit<D, C2> &umax)
{
    return max(min(u, umax), umin);
}

template<typename D, typename C>
inline constexpr Value<D> clamp(const Value<D> &u, const Value<D> &umin,
                                const unit<D, C> &umax)
{
    return max(min(u, umax), umin);
}
...
template<typename D, typename C>
inline constexpr Value<D> clamp(const Value<D> &u, const unit<D, C> &umin,
                                const Value<D> &umax)
{
    return max(min(u, umax), umin);
}
