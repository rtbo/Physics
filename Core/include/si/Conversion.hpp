...
template<typename FR = std::ratio<1>, typename OR = std::ratio<0>,
         int PiExp = 0>
struct conversion
{
    static_assert(detail::is_ratio<FR>(),
                  "conversion must be used with std::ratio");
    static_assert(detail::is_ratio<OR>(),
                  "conversion must be used with std::ratio");
    ...
    {
        return (val + detail::ratio_double<OR>()) * detail::ratio_double<FR>() *
