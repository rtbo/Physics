...
public:
    ...
    constexpr explicit Value() : _repr{ 0.0 } {}

    template<typename C>
    constexpr Value(const unit<D, C> &val) : _repr{ C::conv(val.val()) }
    {}

    // public (implicit) contructor for values without dimensions
    template<typename D_ = D>
    constexpr Value(const double repr,
                    std::enable_if_t<std::is_same_v<D_, D> &&
    ...
    template<typename D_ = D,
             typename = std::enable_if_t<std::is_same_v<D, NoDim> &&
                                         std::is_same_v<D_, D>>>
...
private:
    ...
    friend constexpr bool operator==(const Value<_D> &lhs,
                                     const Value<_D> &rhs);
    ...
    template<typename _D>
    friend constexpr Value<_D> operator+(const Value<_D> &lhs,
                                         const Value<_D> &rhs);
    ...
    template<typename _D>
    friend constexpr Value<_D> operator*(const double &lhs,
                                         const Value<_D> &rhs);
...
inline constexpr auto pow(const Value<D> &val)
{
    ...
    return Value<dim>{ std::pow(val._repr, detail::ratio_double<R>()) };
}
...
template<typename D>
inline constexpr Value<D> clamp(const Value<D> &val, const Value<D> &minVal,
                                const Value<D> &maxVal)
{
